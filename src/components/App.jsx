...
export default function App () {
  ...
  useEffect(() => {
    localStorage.setItem('contactsToLocalStorage', JSON.stringify(contacts));
  }, [contacts]);
  ...
  const filterContacts = () => {
    if (filter) {
      return contacts.filter(
        (contact) =>
          contact.name.includes(filter) || contact.name.toLowerCase().includes(filter)
      );
    } else {
      return contacts;
    }
